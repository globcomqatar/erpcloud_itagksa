...
Every department reviews inside one state, so the Approve and Reject buttons cannot
...
DEPARTMENTS_BY_STATE = {
	"Pending Department Review": ["design", "quality", "operations", "finance"],
...
def record_review(doc, method=None):
	status = ACTION_STATUS.get(doc.flags.get(ACTION_FLAG))
